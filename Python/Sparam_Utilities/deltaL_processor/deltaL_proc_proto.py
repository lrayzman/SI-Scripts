...
#  THIS IS A PROTOTYPE
#
# Performs a deltaL loss and delay calculation from (a set of) s2p or s4p files 
# representing a long and short line lengths
#
# 
...
#      config name 1; ; deltaL 1; long line s*p filename(s) 1, long line s*p filename(s) 2, ..., long line s*p filename(s) n   ; short line s*p filename(s) 1, short line s*p filename(s) 2, ..., short line s*p filename(s) n 
#      config name 2; ; deltaL 2; long line s*p filename(s) 1, long line s*p filename(s) 2, ..., long line s*p filename(s) n   ; short line s*p filename(s) 1, short line s*p filename(s) 2, ..., short line s*p filename(s) n
#        ....
#      config name m; ; deltaL m; long line s*p filename(s) 1, long line s*p filename(s) 2, ..., long line s*p filename(s) n   ; short line s*p filename(s) 1, short line s*p filename(s) 2, ..., short line s*p filename(s) n
#          
# 
#   OUTPUT:
...
def get_sxp_data(filename):
    ...
    loss_vec = np.abs(sp_network.s_mag[:,1,0])
    gdelay_vec = np.real(sp_network.group_delay[:,1,0])        
...
def parse_inlist(in_list):
    ...
    for line in inlist_lines:
        ...
        list_content.update({line[0] : {'long_files' : line[2].split(sep=','),
                                        'short_files' : line[3].split(sep=','),
                                        'deltal' : float(line[1])}})
...
#    
    ...
    band_idx1 = range(np.where(f >= 0.1e9)[0][0], len(f)-1)
...
def run_CMD(in_arg, out_arg):
    ...
    for line_config_name, line_config_content in in_list.items():
        ...
        long_sploss = np.array([])
        short_sploss = np.array([])

        long_gpdly = np.array([])
        short_gpdly = np.array([])
        
                
        # Compute average long loss data
        for sxp_file in line_config_content['long_files']:
            
            freq_vec, loss_vec, gdelay_vec = get_sxp_data(sxp_file.strip())
            
            # essentially initialized the data array
            if long_sploss.size == 0:
                freqs =  np.hstack((freqs, freq_vec))
                long_sploss =  np.hstack((long_sploss, loss_vec))
                long_gpdly = np.hstack((long_gpdly, gdelay_vec))
                
            # otherwise put into existing array
            else:
                if freqs.size == freq_vec.size:
                    long_sploss =  np.vstack((long_sploss, loss_vec))
                    long_gpdly = np.vstack((long_gpdly, gdelay_vec))
                else:        
                    sys.exit('S-parameter data is not matching in frequency points')               
        
        # Compute average short loss data
        for sxp_file in line_config_content['short_files']:
            
            freq_vec, loss_vec, gdelay_vec = get_sxp_data(sxp_file.strip())
            
            # essentially initialized the data array
            if short_sploss.size == 0:
                if freqs.size == freq_vec.size:
                    short_sploss =  np.hstack((short_sploss, loss_vec))
                    short_gpdly = np.hstack((short_gpdly, gdelay_vec))
                else:
                    sys.exit('S-parameter data is not matching in frequency points')
            ...
            # otherwise put into existing array
                ...
                if freqs.size == freq_vec.size:
                    short_sploss =  np.vstack((short_sploss, loss_vec))
                    short_gpdly = np.vstack((short_gpdly, gdelay_vec))
                else:        
                    sys.exit('S-parameter data is not matching in frequency points')
        ...
        loss_vec = 20*np.log10(np.mean(long_sploss, axis = 0) / np.mean(short_sploss, axis = 0)) / line_config_content['deltal']
        magdB_loss_data = pds.DataFrame({'freqs' : freqs, 'loss' : loss_vec})
                
        
        # compute the per inch group delay
        gdelay_vec = (np.mean(long_gpdly, axis = 0) - np.mean(short_gpdly, axis = 0)) / line_config_content['deltal']
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hl:o:", ["help", "listfile=", "outdir="])
        
    except getopt.GetoptError as err:
        # print help information and exit:
        sys.stdout.write(err)  # will print something like "option -a not recognized"
        sys.stdout.write('Usage: deltaL_proc.py -l <s*p list input file> -o <csv output directory>')
        sys.exit(2)
