...
SparErr write_tchstn(char *filename, SParType **spmat)
{
	...
	if((psfiledata=(char*)malloc(sizeof(char)*1024))==NULL)
	{
		return(CreateSpErr(0, errcode104));
	}

	iBuffLen=iBuffLen+1024;
	...
	for (i=0; i<(**spmat).nfreq; i++)
	{
		...
		for(k=1; k<=((**spmat).nport*(**spmat).nport);k++)
		{
			...
			//Running low on memory buffer, need to enlarge
			if(iFilelen >= iBuffLen-100)
			{
				if((psfiledata = realloc(psfiledata, sizeof(char)*(iBuffLen+1024)))== NULL)
				{
	...
	fwrite( psfiledata, sizeof( char ), iFilelen, ffspfile );
