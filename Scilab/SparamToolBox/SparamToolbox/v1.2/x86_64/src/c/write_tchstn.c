...
//                             to file 
//                07/22/2015 - Added option to write user comments in th header
// ====================================================================
/* ==================================================================== */
...
/* ==================================================================== */
SparErr write_tchstn(char *filename, SParType **spmat, char *StrComments)
{
	...
	int i, j, k, l, m;                      // Just some tem variables
    ...
    if(StrComments)
	{
	    k=strlen(StrComments);

		if(k!=0)
		{
			l=1;   //Init index of appended array for first comment marker		
			/* First pass is to figure a comment token at every line */
			for (i=0; i<k; i++)
			{
				l++;
				if(StrComments[i]=='\n')
  			...
  			/* Allocate memory for temporary storage */
			if((pStrComments=(char*)calloc(l+1, sizeof(char)))==NULL)
			{
				return(CreateSpErr(0, errcode104));
			}

			l=0;
			/* Second pass is to copy original text and  */
			/*    pre-pend a comment marker on a newline */
			pStrComments[l]='!';
			for (i=0; i<k; i++)
			{
				l++;
				pStrComments[l]=StrComments[i];
				if(StrComments[i]=='\n')
				{
					l++;
					pStrComments[l]='!';
			...
			/* Add comments */
			j = sprintf((psfiledata+iFilelen), "%s\r\n", pStrComments);
			iFilelen=iFilelen+j;
