...
[ok,refdata_scalar]=getvalue(["Length scaling factor"; ""; "Example: 1 meter : 1 foot = 3.28"],"", list("vec",1),"1.00");
...
///////////////////
    ...
    plot2d(spreffreqs/1e9, spfitdata, style=7);     //lower  children(3)
     ...
     plot2d(spreffreqs/1e9, 20*log10(abs(sprefdata_row_col))/refdata_scalar);    // children(2)   // Reference magnitude plot
    ...
    xtitle(strcat(["S(DD)" string([srow scol])]), "Frequency(GHz)", "Magnitude(dB)");       
    ploth.title.font_size=3;
    ploth.title.font_style=8;
                                                                                                                                                // Legend                                          
    ...
    uicontrol("style", "text", "string", "ILD f-factor", "FontWeight", "bold", "BackgroundColor", [1 1 1], "FontSize", 12, "Position", [475 40 75 25]); 
