...
Pn_L_fit=[];                                // Phase noise curve fit
h_coeff=[];                                 // Coefficients of fit curve
h_coeff0=[;0;0;0];                       // Initial coefficient values
...
// 
// Note this has to compute instantenous frequency spectrual density 10log10[Sy(f)]
// 
[Pn_L_fit_e, h_coeff]=leastsq(list(errfunc, Pn_f, 10*log10(Pn_L)), h_coeff0);    
Pn_L_fit=10^((xfit(Pn_f, h_coeff))/10);
clear Pn_L_fit_e;
...
plot2d(Pn_f/freqscalar, 10*log10(Pn_L_fit),logflag="ln");              
